function [gradX, gradZ, gradU, dbg] = grad_score(sd)
...
dScore_dK = implicit.dScore_dK(sd,K,P); % m×n
...
dScore_dD = dScore_dK*(dFk_dL*dL_dD + dFk_dD);

% Reshape to match D structure: D = [Z; X; U] is (2*n+m) × T
% dRho_dD is dvec(ρ)/dvec(D), which is 1 × ((2*n+m)*T) after computation
bar_n = 2*n + m;
dScore_dD = reshape(dScore_dD, bar_n, T);  % (2*n+m) × T

% Extract gradients for each block: D = [Z; X; U]
gradZ = dScore_dD(1:n, :);           % n × T
gradX = dScore_dD(n+1:2*n, :);       % n × T
gradU = dScore_dD(2*n+1:2*n+m, :);   % m × T
...
if nargout > 3
    ...
    dbg.K = K; dbg.lam = lam;
    dbg.dScore_dK = dScore_dK;
    dbg.dScore_dD = dScore_dD;
    dbg.kkt = kkt;
end
end
